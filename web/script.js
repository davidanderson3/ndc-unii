...
(function () {
  ...
  function render(records, query) {
    ...
    for (const rec of grouped) {
      ...
      if (rec.ndcs && rec.ndcs.length > 1) {
        ...
        const ndcList =
          rec.ndcs && rec.ndcs.length ? rec.ndcs.join(", ") : rec.ndc;
      ...
      byIN = byIN
        .slice()
        .sort((a, b) =>
          (a.str || "").localeCompare(b.str || "", undefined, {
            sensitivity: "base",
          }),
        );
      byPIN = byPIN
  ...
  function filterRecords(records, query) {
    ...
    return records.filter((rec) => {
      ...
      const nameMatch = qTokens.length
  ...
  async function onInput() {
    ...
    try {
      if (!qd && !hasLetters) {
        setStatus("Ready");
        resultsEl.innerHTML =
          '<div class="hint">Enter an NDC (3+ digits) or search by name, RxCUI, or UNII.</div>';
        return;
      }
      ...
      if (!hasLetters && qd.length > 0 && qd.length < 3) {
        ...
        resultsEl.innerHTML =
          '<div class="hint">Enter at least the first 3 NDC digits.</div>';
        return;
      }
      ...
      try {
        ...
        for (const rec of idx.records || []) {
          ...
          const ndcDigits = digits(rec.ndc);
          const ndcMatch =
            !hasLetters && qd.length >= 3 ? ndcDigits.includes(qd) : false;
          const nameMatch = qTokens.length
            ? qTokens.every((t) => name.includes(t))
            : qLower && name.includes(qLower);
        ...
        setStatus(
          "Search index missing; ensure web/data/search_index.json exists",
        );
        render([], qTrim);
        return;
      }
  ...
  if (initialQ) {
    ...
    setStatus("Ready");
    resultsEl.innerHTML =
      '<div class="hint">Provide chunks in web/data via the build script, then search by NDC, name, RxCUI, or UNII.</div>';
  }
  ...
  const examples = [
    {
      ...
      items: [
        {
          query: "00002-7715-01",
          text: "00002-7715-01 (insulin glargine pen)",
        }, // insulin glargine
        {
          query: "00003-0893-21",
          text: "00003-0893-21 (apixaban 2.5 mg tablet)",
        }, // apixaban
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        {
          query: "197886",
          text: "197886 (lisinopril/hydrochlorothiazide 20/12.5)",
        }, // lisinopril/HCTZ
